...
public class Rectangle {
    ...
    public boolean canForm(ArrayList<Leaf> nodes) {
        ...
        HashSet<Leaf> combinations = new HashSet<>();
        ...
        for (int i = 0; i <=originalSize ; i++) {
            ...
            for (int q = 0; q < currentSize; q++) {
                ...
                for (int w = q + 1/*هون بلشنا من بعد الاولى */; w < currentSize; w++) {
                    ...
                    if (!nodes.get(q).equals(nodes.get(w))) {
                        ...
                        if (nodes.get(q).length == nodes.get(w).length && !containSameCapitalLetters(nodes.get(q).name, nodes.get(w).name)) {//هاد الشرط للتعامل مع موضوع الاسم اذا كان سترينغ ولا تشار
                            combinations.add(new Leaf(
                ...
                for (int w = q + 1; w < currentSize; w++) {
                    if (!nodes.get(q).equals(nodes.get(w))) {
                        if (nodes.get(q).width == nodes.get(w).width && !containSameCapitalLetters(nodes.get(q).name, nodes.get(w).name)) {
                            combinations.add(new Leaf(
                                    "(" + nodes.get(q).name + "|" + nodes.get(w).name + ")",
                                    nodes.get(q).length + nodes.get(w).length,
                                    nodes.get(q).width
                            ));
                        }
                    }
            ...
            Set<Leaf> set = new HashSet<>(nodes);
        ...
        for (Leaf leaf : nodes) {
            if (countUppercaseLetters(leaf.name) == originalSize) {
